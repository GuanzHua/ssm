...
public class LogAop {
    ...
    @Before("execution(* com.guanzh.controller.*.*(..))")
    public void doBefore(JoinPoint joinPoint) {
        ...
        /*if (args == null || args.length == 0) {
            ...
            for (int i = 0; i < args.length; i++) {     //ModelMap  BindingAwareModelMap
                classArgs[i] = args[i].getClass();
            }
            method = joinPoint.getSignature().getDeclaringType().getMethod(methodName, classArgs);
        }*/

    }

    //后置通知
    @After("execution(* com.guanzh.controller.*.*(..))")
